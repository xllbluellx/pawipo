...
class Anexo_2{
	...
	public function anexo2($alumno) {
		...
		$this->conn = new Conexion('../../php/datosServer.php');
		$this->conn = $this->conn->conectar();	
		$sql = "SELECT respuestas.respuestas FROM respuestas WHERE respuestas.idAnexo = 2 AND respuestas.subAnexo = 1 AND respuestas.idAlumno = ".$alumno;
		$result = $this->conn->query($sql);
            if ($result->num_rows > 0) {
                while($row = $result->fetch_array(MYSQLI_NUM)) {  
                   $array = json_decode($row[0], true);
...
class Anexo_3{
	...
	public function anexo3($alumno) {
		...
		$this->conn = new Conexion('../../php/datosServer.php');
		$this->conn = $this->conn->conectar();	
		$sql = "SELECT respuestas.respuestas, respuestas.subAnexo FROM respuestas WHERE respuestas.idAnexo = 3 AND respuestas.idAlumno = ".$alumno." ORDER BY respuestas.subAnexo ASC";
		$result = $this->conn->query($sql);
            if ($result->num_rows > 0) {
                while($row = $result->fetch_array(MYSQLI_NUM)) {  
                   $array = json_decode($row[0], true);
...
class Anexo_4{
	...
	public function anexo4($alumno) {
		...
		$this->conn = new Conexion('../../php/datosServer.php');
		$this->conn = $this->conn->conectar();	
		$sql = "SELECT respuestas.respuestas FROM respuestas WHERE respuestas.idAnexo = 4 AND respuestas.subAnexo = 1 AND respuestas.idAlumno = ".$alumno;
		$result = $this->conn->query($sql);
            if ($result->num_rows > 0) {
                while($row = $result->fetch_array(MYSQLI_NUM)) {  
                   $array = json_decode($row[0], true);
...
class Anexo_5{
	...
	public function anexo5($alumno) {
		...
		$this->conn = new Conexion('../../php/datosServer.php');
		$this->conn = $this->conn->conectar();	
		$sql = "SELECT respuestas.respuestas FROM respuestas WHERE respuestas.idAnexo = 5 AND respuestas.subAnexo = 1 AND respuestas.idAlumno = ".$alumno;
		$result = $this->conn->query($sql);
            if ($result->num_rows > 0) {
                while($row = $result->fetch_array(MYSQLI_NUM)) {  
                   $array = json_decode($row[0], true);
...
class Anexo_6{
	...
	public function anexo6($alumno) {
		...
		$this->conn = new Conexion('../../php/datosServer.php');
		$this->conn = $this->conn->conectar();	
		$sql = "SELECT respuestas.respuestas FROM respuestas WHERE respuestas.idAnexo = 6 AND respuestas.subAnexo = 1 AND respuestas.idAlumno = ".$alumno;
		$result = $this->conn->query($sql);
            if ($result->num_rows > 0) {
                while($row = $result->fetch_array(MYSQLI_NUM)) {  
                   $array = json_decode($row[0], true);
...
class Anexo_7{
	...
	public function anexo7($alumno) {
		...
		$this->conn = new Conexion('../../php/datosServer.php');
		$this->conn = $this->conn->conectar();	
		$sql = "SELECT respuestas.respuestas FROM respuestas WHERE respuestas.idAnexo = 7 AND respuestas.subAnexo = 1 AND respuestas.idAlumno = ".$alumno;
		$result = $this->conn->query($sql);
            if ($result->num_rows > 0) {
                while($row = $result->fetch_array(MYSQLI_NUM)) {  
                   $array = json_decode($row[0], true);
...
class Anexo_8{
	...
	public function anexo8($alumno) {
		...
		$this->conn = new Conexion('../../php/datosServer.php');
		$this->conn = $this->conn->conectar();	
		$sql = "SELECT respuestas.respuestas FROM respuestas WHERE respuestas.idAnexo = 8 AND respuestas.subAnexo = 1 AND respuestas.idAlumno = ".$alumno;
		$result = $this->conn->query($sql);
            if ($result->num_rows > 0) {
                while($row = $result->fetch_array(MYSQLI_NUM)) {  
                   $array = json_decode($row[0], true);
	...
	// --------------- Constancias: Genera conctancias de tutorados
class Constancia{
		private $conn;
	public $t1;
	public $t2;
	...
	public function Tutorado($alumno) {
		$encontrado = false;
		$header = '<br><br><div style="text-align: right;">DEPENDENCIA: SUBD. ACADÉMICA<br>SECCIÓN: DESARROLLO ACADÉMICO<br><br /><br />';		
		$header .= $this->fecha();
		$header .= "<br><br>Asunto: <b>Liberación de crédito complementario.</b></div>";
		 
		$cont = '';
		$this->conn = new Conexion('../../php/datosServer.php');
		$this->conn = $this->conn->conectar();	
		$sql = "CALL constanciaTutorado(".$alumno.", '".$_SESSION['tutor']."');";
		$result = $this->conn->query($sql);
            if ($result->num_rows > 0) {
            	$encontrado = true;
                while($row = $result->fetch_array(MYSQLI_NUM)) {  
                   $cont = $this->formato($row[0], $row[1], $row[2], $row[3], $alumno); 
                   break;        	
                	}
            }
	...
	public function TutoradoAdmin($alumno) {
		$encontrado = false;
		$header = '<br><br><div style="text-align: right;">DEPENDENCIA: SUBD. ACADÉMICA<br>SECCIÓN: DESARROLLO ACADÉMICO<br><br /><br />';		
		$header .= $this->fecha();
		$header .= "<br><br>Asunto: <b>Liberación de crédito complementario.</b></div>";
		...
		$this->conn = new Conexion('../../php/datosServer.php');
		$this->conn = $this->conn->conectar();	
		$sql = "CALL constanciaTutoradoAdmin(".$alumno.");";
		$result = $this->conn->query($sql);
            if ($result->num_rows > 0) {
            	$encontrado = true;
                while($row = $result->fetch_array(MYSQLI_NUM)) {  
         ...
         // Inicia llenado de Formato
			...
			return '<br><br><br><br><div style="text-align: center; color: red;">ERROR: verificar si el alumno tiene tutor.</div>';     
	}
	
	public function Tutor($grupo) {
		$encontrado = false;
		$header = '<br><br><div style="text-align: right;">DEPENDENCIA: SUBD. ACADÉMICA<br>SECCIÓN: DESARROLLO ACADÉMICO<br><br /><br />';		
		$header .= $this->fecha();
		$header .= "<br><br>Asunto: <b>Constancia Tutorías.</b></div>";
		 
		$cont = '';
		$this->conn = new Conexion('../../php/datosServer.php');
		$this->conn = $this->conn->conectar();	
		$sql = "CALL constanciaTutor('".$grupo."');";
		$result = $this->conn->query($sql);
            if ($result->num_rows > 0) {
            	$encontrado = true;
                while($row = $result->fetch_array(MYSQLI_NUM)) {  
                   $cont = $this->formatoTut($row[0], $row[1], $row[2], $row[3]); 
                   break;        	
                	}
            }
	...
	public function Coordinador($carrera) {
		$encontrado = false;
		$header = '<br><br><div style="text-align: right;">DEPENDENCIA: SUBD. ACADÉMICA<br>SECCIÓN: DESARROLLO ACADÉMICO<br><br /><br />';		
		$header .= $this->fecha();
		$header .= "<br><br>Asunto: <b>Constancia Tutorías.</b></div>";
		...
		$this->conn = new Conexion('../../php/datosServer.php');
		$this->conn = $this->conn->conectar();	
		$sql = "CALL constanciaCoor('".$carrera."');";
		$result = $this->conn->query($sql);
            if ($result->num_rows > 0) {
            	$encontrado = true;
                while($row = $result->fetch_array(MYSQLI_NUM)) {  
                   $cont = $this->formatoCor($row[0], $row[1]); 
                   break;        	
                	}
            }
	...
	public function fecha(){
		...
		return "<br>Morelia, Mich., a ".$dia." de ".$mes." del ".$anio;
	}

	public function formato($nom, $grupo, $carrera, $profe, $noc){
		$datos = '<br><br><br>A QUIEN CORRESPONDA:<br><br>';
		
		$info = explode("-", $grupo);
		
		switch(intval($info[2])) {
				case 1: $per = 'ENERO-JULIO'; break;
				case 2: $per = 'VERANO'; break;
				case 3: $per = 'AGOSTO-DICIEMBRE'; break;	 
			}		
		
		$datos .= '<span style="text-align: justify;">La que suscribe Jefa del Departamento de Desarrollo Académico por  medio del presente, 
		hago constar que el (la) <b>C. '.strtoupper($nom).' </b>alumno(a) de nuestro instituto con número de control  <b>'.$noc.'</b>  de 
		la carrera  <b>'.strtoupper($carrera).'</b> asistió a tutorías durante el semestre <b>'.$per.' '.$info[1].'</b>, siendo su tutor el (la) C. Profesor (a) <b>'.strtoupper($profe).'</b>.<br><br><br>';
		
		$datos .= 'Se extiende la presente con la finalidad de que le sea acreditado un crédito complementario en base a acuerdo con el Comité Académico. 
		Sin más por el momento, quedo a sus órdenes para cualquier aclaración.</span>';
		
		$datos .= '<br><br><br><br><br><br><b>Atentamente<br><br><br>L. I. Rocío Contreras Jiménez<br>Jefa del Departamento de Desarrollo Académico</b>';
		$datos .= '<br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><img src="../../img/footer.png" style="text-align: center;" />';
		return $datos;
	}
	
	public function formatoTut($nom, $carrera, $total, $grupo){
		$datos = '<br><br><br>A QUIEN CORRESPONDA:<br><br>';
		...
		$datos .= '<span style="text-align: justify;">Le informo que el (la) <b>C. Profesor (a) '.strtoupper($nom).'</b>, 
		de la carrera de <b>'.strtoupper($carrera).'</b> participó como tutor en el semestre '.$per.' '.$info[1].', 
		atendiendo un total de '.$total.' alumnos.<br><br><br><br>Se extiende la solicitud a petición del interesado,
		 para los fines legales que juzgue convenientes.</span>';
		
		$datos .= '<br><br><br><br><br><br><table style="text-align: center;"><tr><th colspan="2"><b>Atentamente</b><br><br><br><br></th></tr>
		<tr><td>________________________________<br>L. I. Rocío Contreras Jiménez<br><br><br><b>Jefa del Departamento de Desarrollo Académico</b></td>
		<td>________________________________<br>Dr. Rafael Lara Hernández<br><br><br><b>Subdirector Académico</b></td>		
		</tr>		
		</table>';
		
		$datos .= '<br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><img src="../../img/footer.png" style="text-align: center;" />';
		return $datos;
	}
	
	public function formatoCor($nom, $carrera){
		$datos = '<br><br><br>A QUIEN CORRESPONDA:<br><br>';
			
		
		$datos .= '<span style="text-align: justify;">Le informo que el (la) <b>C. Profesor (a) '.strtoupper($nom).'</b>, 
		de la carrera de <b>'.strtoupper($carrera).'</b> desempeño actividades de Coordinador (a) de Tutorías de su 
		Academia en el semestre (Falta Info).<br><br><br><br>Se extiende la solicitud a petición del interesado,
		 para los fines legales que juzgue convenientes.</span>';
