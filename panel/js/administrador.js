$(document).ready(function () {
	...
	// boton en imprimir > evaluaciones
	$("div#ContenedorPrincipal").on('click', 'button#con-eva',function (e) {
		  e.preventDefault();
		  var form = $(this).closest('form').attr('id');
		  if (evaluarModal(form)) {
		  			var carrera = $('div#ContenedorPrincipal').find('form#'+form+" select#carrera").val();
		  			var anio = $('div#ContenedorPrincipal').find('form#'+form+" input#anio").val();
		  			consultarDatos(carrera, anio);
		  	}
	});
	// click en boton guardar cualquier boton
	...
	//click en foro
	/*$("div#ContenedorPrincipal").on('click', 'button#foro',function (e) {
		  e.preventDefault();
		  var form = $(this).closest('form').attr('id');
		  foro(form);
	});*/
...
// ---- Funcion para obtener los tutores mediante AJAX 
	...
	function consultarDatos(dato1, dato2) {
                ...
                success: function (infoRegreso) {
                	switch (parseInt(infoRegreso)) {
                		case -1:
							...
							break;
							default: $('div#ContenedorPrincipal div#tablas').html(infoRegreso);
                		}
                	}
            });
		}
...
//Evalua todos los campos de cualquier modal form.
	function evaluarModal(modal) {
		...
		switch($.cookie('subop')) {	
			...
			break;
			case 'eval':
				if (!validarEvaluacion(inputs, ids, modal)) return false; 
				else return true;
			break;
			case 'change-pass':
	...
	function validarAlumnoEdit(inputs, ids, modal) {
		...
		// -------------------------------------------	
	function validarEvaluacion(inputs, ids, modal) {
		for (var i = 0; i < inputs.length; i++) {
			if (inputs[i].length == 0) {
				$('div#ContenedorPrincipal').find("form#"+modal+" input#"+ids[i]).addClass('error');
				alertify.error('<i class="fa fa-warning"></i> No puedes dejar campos vacios.');
				return false;
				}
		}
		return true;
